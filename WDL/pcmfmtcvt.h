...
#define float_TO_UINT8 double_TO_UINT8
...
{
  out = (T) ((in - 128) * (1.0 / 128.0));
}

template<class T> static void double_TO_UINT8(T &out, double in)
{
  in *= 128.0;
  if (in <= -128.0) out = 0;
  else if (in >= 127.0) out = 255;
  else out = (T) ((int) floor(in + 0.5) + 128);
}
